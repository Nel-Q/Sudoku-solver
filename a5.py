...
class Board:
    ...
    def find_most_constrained_cell(self) -> Tuple[int, int]:
        ...
        for row in range(self.size):
            for col in range(self.size):
                if isinstance(self.rows[row][col], list):
                    if len(self.rows[row][col]) < shortest_len:
                        shortest_len = len(self.rows[row][col])
                        cell = (row,col)
    ...
    def update(self, row: int, column: int, assignment: int) -> None:
        ...
        lstupl = self.subgrid_coordinates(row,column)
        print(lstupl)

        for tup in lstupl:
            remove_if_exists(self.rows[tup[0]][tup[1]],assignment)

##        rowx = (row//3)*3
##        rowc = (column//3)*3
##        for i in range(3):
##            remove_if_exists(self.rows[rowx+i][column],assignment)
##        for j in range(3):
##            remove_if_exists(self.rows[row][rowc+j],assignment)
...
def DFS(state: Board) -> Board:
    ...
    while not mys.is_empty():
        curr = mys.the_stack[-1]
        if curr.goal_test()== True:
            return curr
        elif curr.failure_test() == False:
            (row,col) = curr.find_most_constrained_cell()
            #print((row,col))
            for i in curr.rows[row][col]:
                #curr.rows[row][col] = i
                brd_copy = copy.deepcopy(curr)
                brd_copy.update(row,col,i)
                mys.push(brd_copy)
    return None
